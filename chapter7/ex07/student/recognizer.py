...
def sentence(words):
    """Recognize a sentence with one or more independent clauses."""
    success, remainder = simple_sentence(words)
    if not success:
        return False, words

    # Recursive rule: if next word is a conjunction, expect another sentence
    while remainder and remainder[0] in conjunctions:
        success2, remainder2 = sentence(remainder[1:])
        if success2:
            remainder = remainder2
        else:
            return False, words  # conjunction not followed by valid clause
    ...
    if words and words[0] in verbs:
        ...
        if success:
            ...
            if remainder and remainder[0] in prepositions:
                ...
                if success2:
                    return True, remainder2
            # allow verb phrase without prepositional phrase
            return True, remainder
    return False, words
